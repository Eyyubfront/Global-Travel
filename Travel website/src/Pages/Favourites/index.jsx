...
import { Box, Stack, Typography, IconButton, Button, Dialog, DialogActions, DialogContent, DialogTitle } from '@mui/material';
        ...
        <>
            <Box padding="20px">
                ...
                {favorites.length === 0 ? (
                    ...
                    <Stack className='favorit_cards' gap="20px" flexDirection="row" alignItems="center" flexWrap="wrap">
                        {favorites.map(item => (
                            <Box key={item.id} className="favorite-item">
                                <Stack flexDirection="column">
                                    ...
                                    <Stack gap="2px" flexDirection="column" className='cards_container'>
                                        ...
                                        <Stack className='fav_containers'>
                                            ...
                                            <Stack justifyContent="center" alignItems="center" flexDirection="row">
                                                <Stack gap="10px" alignItems="center" flexDirection="column">
                                                    <Typography className='pricesfavorite'>${item.price}</Typography>
                                                    <Typography>({item.days} days)</Typography>
                                                </Stack>
                                            </Stack>
                                        </Stack>
                                        <Stack className='fav_butom' alignItems="center" flexDirection="row">
                                            <IconButton onClick={() => handleOpenDialog(item.id)} aria-label="remove from favorites">
            ...
            <Dialog open={openDialog} onClose={handleCloseDialog} className="custom-dialog">
                ...
                <DialogContent className="custom-dialog-content">
                    <Typography>Are you sure you want to remove this item from favorites?</Typography>
                </DialogContent>
                <DialogActions className="custom-dialog-actions">
                    <Button onClick={handleCloseDialog} color="primary" className="cancel-button">
                        Cancel
                    </Button>
                    <Button onClick={() => handleRemoveFavorite(selectedFavoriteId)} color="secondary" className="delete-button">
                        Remove
                    </Button>
                </DialogActions>
            </Dialog>
